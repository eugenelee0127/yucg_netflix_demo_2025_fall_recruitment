/* Netflix Landing/Hover Prototype — "The Shawshank Redemption"
   ...
   - Commenting is enabled only in the "Uncensored Window" (watch progress 95%–99%).
   - Public feed releases only spoiler-free comments; spoilers appear only in the Uncensored feed.
*/

/* Use React from the UMD global (no imports/exports needed) */
const { useMemo, useState } = React;

const UNLOCK_START = 95; // unlock uncensored commenting after the story is effectively finished
const CREDITS_START = 100; // just before credits; window closes at credits
...
function App() {
  ...
  const [activeTab, setActiveTab] = useState("public"); // 'public' | 'uncensored'
  ...
  const publicComments = useMemo(
    () => comments.filter((c) => !c.containsSpoiler),
    [comments]
  );

  const uncensoredComments = comments; // all comments visible when window is open
    ...
    return (
      <div
        className={classNames(
          "rounded-2xl border border-white/10 bg-white/5 p-3 md:p-4",
          hidden && "opacity-60"
        )}
      >
        ...
        {hidden && (
          <div className="mt-2 text-xs text-white/60">
            Hidden in Public feed due to spoilers. Visible in Uncensored Window.
          </div>
        )}
      </div>
    );
    ...
    <div className="min-h-screen bg-neutral-950 text-white">
      ...
      <header className="sticky top-0 z-40 border-b border-white/10 bg-neutral-950/80 backdrop-blur supports-[backdrop-filter]:bg-neutral-950/60">
        <div className="mx-auto max-w-7xl px-4 py-3 md:py-4 flex items-center gap-3">
          ...
          <span className="ml-auto text-xs md:text-sm text-white/60">Uncensored comment window: {UNLOCK_START}%–{CREDITS_START - 0.01}%</span>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 md:py-10 grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <section className="lg:col-span-2">
          <div
            className="group relative aspect-[2/3] w-full overflow-hidden rounded-2xl bg-gradient-to-br from-neutral-800 to-neutral-900 shadow-2xl ring-1 ring-white/5"
            onMouseEnter={() => setHovered(true)}
            onMouseLeave={() => setHovered(false)}
          >
            {/* Poster placeholder */}
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(255,255,255,0.12),transparent_40%),radial-gradient(circle_at_70%_80%,rgba(255,255,255,0.08),transparent_40%)]" />
            <div className="absolute inset-0 p-4 flex flex-col justify-between">
              <div className="flex items-center gap-2">
                <span className="rounded bg-white/10 px-2 py-0.5 text-[10px] uppercase tracking-wide text-white/80">Drama</span>
                <span className="text-[10px] text-white/50">1994 • 2h 22m • R</span>
              </div>
              <div>
                <h2 className="text-xl md:text-2xl font-bold">The Shawshank Redemption</h2>
                <p className="mt-1 text-xs text-white/70">Tim Robbins • Morgan Freeman • Bob Gunton</p>
              </div>
            </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4 md:p-6">
            ...
            <div className="mt-4">
              ...
              <div className="mt-2 flex items-center justify-between text-[11px] text-white/60">
                ...
                <span>Credits at <strong>{CREDITS_START}%</strong></span>
              </div>
            </div>
              ...
              <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                <label className="flex items-center gap-2 text-sm">
                  <input
                    type="checkbox"
                    className="h-4 w-4"
                    checked={spoilerFilterOn}
                    onChange={(e) => setSpoilerFilterOn(e.target.checked)}
                  />
                  <span className="font-medium">Spoiler filter (Public feed)</span>
                </label>
                <p className="mt-2 text-xs text-white/70">
                  Public feed always hides spoilers. Uncheck to preview moderator view.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5">
            ...
            <div className="p-4 md:p-5 border-b border-white/10">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <textarea
                    ...
                    placeholder={
                      windowOpen
                        ? "Write your comment (spoilers allowed in Uncensored Window)..."
                        : "Commenting locked until Uncensored Window (95%–99%)"
                    }
                  ...
                  <div className="mt-2 flex items-center justify-between">
                    <div className="text-[11px] text-white/60">
                      {draft && (
                        <span>
                          {detectSpoiler(draft)
                            ? "Detected possible spoilers: will NOT appear in Public feed."
                            : "No spoilers detected: eligible for Public feed."}
                        </span>
                      )}
                    </div>
                    <button
              ...
              <p className="mt-2 text-[11px] text-white/60">
                Policy: during the Uncensored Window, all comments (including spoilers) are accepted. After moderation, only spoiler-free comments are released to Public.
              </p>
            </div>

            {/* Feeds */}
            <div className="p-4 md:p-6 space-y-3">
              {activeTab === "public" && publicComments.length === 0 && (
                <p className="text-sm text-white/60">No public comments yet.</p>
              )}
              {activeTab === "public" &&
                publicComments.map((c) => <Comment key={c.id} c={c} />)}
              {activeTab === "uncensored" && !windowOpen && (
                <div className="rounded-xl border border-white/10 bg-white/5 p-4 text-sm text-white/70">
                  The Uncensored Window is locked. Move the progress slider to {UNLOCK_START}%–{CREDITS_START - 0.01}% to preview it.
                </div>
              )}
              {activeTab === "uncensored" && windowOpen &&
                uncensoredComments.map((c) => <Comment key={c.id} c={c} />)}
            </div>
          </div>
        </aside>
      </main>
      ...
      <footer className="mx-auto max-w-7xl px-4 pb-10">
        <div className="rounded-2xl border border-white/10 bg-white/5 p-4 md:p-6 text-sm text-white/70">
          ...
          <ul className="mt-2 list-disc space-y-1 pl-5">
            <li>
              <span className="font-medium">Uncensored commenting window</span> — gated by watch progress (95%–99%).
            </li>
            <li>
              <span className="font-medium">Evaluative summary</span> — concise, neutral summary placed above the comment feeds.
            </li>
            <li>
              <span className="font-medium">Public release</span> — spoiler-free comments appear in the Public feed.
            </li>
          </ul>
        </div>
      </footer>
